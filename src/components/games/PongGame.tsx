...
const PongGame = () => {
  ...
  // Use refs for game loop state to avoid re-running effects
  const gameStateRef = useRef({
    ballPos: { x: CANVAS_WIDTH / 2, y: CANVAS_HEIGHT / 2 },
    ballVel: { x: INITIAL_BALL_SPEED, y: INITIAL_BALL_SPEED },
  ...
  const resetGame = useCallback(() => {
    ...
    // Reset refs
    gameStateRef.current.ballPos = { x: CANVAS_WIDTH / 2, y: CANVAS_HEIGHT / 2 };
    gameStateRef.current.ballVel = { x: INITIAL_BALL_SPEED, y: Math.random() * 2 - 1 };
  ...
  useEffect(() => {
    ...
    const gameLoop = setInterval(() => {
      ...
      const diff = state.ballPos.y - aiCenter;
      const speed = Math.min(PADDLE_SPEED * 0.7, Math.abs(diff));
      if (diff > 5) state.aiY = Math.min(CANVAS_HEIGHT - PADDLE_HEIGHT, state.aiY + speed);
      else if (diff < -5) state.aiY = Math.max(0, state.aiY - speed);

      // Ball movement
      let newX = state.ballPos.x + state.ballVel.x;
      let newY = state.ballPos.y + state.ballVel.y;
      let newVelX = state.ballVel.x;
      let newVelY = state.ballVel.y;
      ...
      if (
        ...
        newY + BALL_SIZE >= state.playerY &&
        newY <= state.playerY + PADDLE_HEIGHT
      ) {
        newVelX = Math.abs(newVelX) * 1.05;
        const hitPos = (newY - state.playerY) / PADDLE_HEIGHT;
      ...
      if (
        ...
        newY + BALL_SIZE >= state.aiY &&
        newY <= state.aiY + PADDLE_HEIGHT
      ) {
        newVelX = -Math.abs(newVelX) * 1.05;
        const hitPos = (newY - state.aiY) / PADDLE_HEIGHT;
        newVelY = (hitPos - 0.5) * 6;
        newX = CANVAS_WIDTH - PADDLE_WIDTH - 10 - BALL_SIZE;
        playSound('paddle');
      }

      // Score logic needs to interact with React State (carefully)
      if (newX <= 0) {
        setAiScore(s => {
          const newScore = s + 1;
          if (newScore >= 5) {
            ...
            setIsPlaying(false);
            playSound('gameover');
            saveScore('pong', playerScore); // Use captured playerScore? No, might be stale.
            // Actually, we can just save `playerScore` from state if we trust it doesn't change during this tick.
            // But better to not save here or use a ref for score too if we want perfect accuracy
          } else {
            playSound('error');
          }
          return newScore;
        ...
        newVelX = INITIAL_BALL_SPEED; // Reset speed
        newVelY = (Math.random() - 0.5) * 2; // Random Y
        ...
        newX = CANVAS_WIDTH / 2;
        newY = CANVAS_HEIGHT / 2;
        newVelX = -INITIAL_BALL_SPEED; // Reset speed towards player
        newVelY = (Math.random() - 0.5) * 2;
      }

      // Update refs
      state.ballPos = { x: newX, y: newY };
      state.ballVel = { x: newVelX, y: newVelY };

      // Sync to state for render (only what's needed for UI)
      setPlayerY(state.playerY);
      setAiY(state.aiY);
      setBallPos(state.ballPos);

    }, 1000 / 60);

    return () => clearInterval(gameLoop);
  }, [isPlaying, gameOver, playSound, playerScore]); // minimal deps
      ...
      <div className="flex gap-2">
        <Button
          ...
          onClick={() => {
            ...
            if (!isPlaying) {
              // Sync refs to current state when restarting/unpausing if needed
              // logic mostly handles it in resetGame
            }
          }}
          disabled={gameOver}
